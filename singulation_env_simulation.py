...
class SingulationEnv:
    ...
    def save(self):
        ...
        summary = np.array([[obj.body.position[0], obj.body.position[1], obj.body.angle] for obj in self.objs])
        return summary
    ...
    def sample(self, num_steps, prune_method, metric, display=False, path=None):
        ...
        for i in range(num_steps):
            ...
            if path is not None:
                self.step(vec[0], vec[1], display=display, path=path+str(i))
            else:
                self.step(vec[0], vec[1], display=display, path=path)
        final_score = metric()
...
if __name__ == "__main__":
    ...
    while True:
        try:
            test_env.create_random_env(num_obj)
            break
        except:
            print("retry")
    before_summary = test_env.save()
    final_score, actions, final_state = test_env.sample(num_steps=num_steps, prune_method=no_prune,
                                                        metric=test_env.count_soft_threshold, display=False,
                                                        path="./sampling")
    ...
    for i in range(num_steps):
        ...
        test_env.step(start_pt, end_pt, display=False, path="./push" + str(i))
    # print("final state")
    # print(final_state)
    # print("env.save()")
    # print(test_env.save())
    np.testing.assert_allclose(final_state, test_env.save())
    assert final_score == test_env.count_soft_threshold()
