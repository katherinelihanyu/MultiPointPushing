...
class State:
    ...
    def greedy_step(self, prune_method, metric, sample_size=None):
        ...
        for i in indices:
            ...
            action = pushes[i]
            self.push(action, display=False, path="sample_step2_0")
            curr_score = metric()
    ...
    def sample_best(self, step, num_steps, num_sample, sample_func):
        best_result = 0
        best_push = None
        best_state = None
        ...
        for _ in range(num_sample):
            ...
            result, action, state, first_step_return, first_step_end_state = sample_func(sample_env, sampled, step, num_steps)
...
if __name__ == "__main__":
    ...
    result = env.sample_closed_loop(num_steps=NUM_STEPS, num_sample=1, sample_func=lambda sample_env, sampled, step, num_steps: sample_env.sample(num_steps=num_steps, prune_method=no_prune, metric=sample_env.count_soft_threshold, sampled=sampled, display=True, path="sample_step%d_" % step))
    print("result", result)
